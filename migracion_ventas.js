...
async function run() {
    ...
    try {
        ...
        await client.query(`ALTER TABLE fg_facturacion ALTER COLUMN certificado_id DROP NOT NULL`);
        
        // Add venta_id column if not exists
        const checkVentaCol = await client.query(`
            SELECT column_name 
        ...
        if (checkVentaCol.rowCount === 0) {
            await client.query(`ALTER TABLE fg_facturacion ADD COLUMN venta_id BIGINT REFERENCES fg_venta(id)`);
        }
