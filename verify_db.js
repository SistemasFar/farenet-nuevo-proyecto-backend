...
async function verify() {
  try {
    const insp = await pool.query("SELECT nrodocumentoinspeccion, comprobante_id, vehiculo_nromotor, fechcreacion FROM inspeccion ORDER BY fechcreacion DESC LIMIT 1");
    const latestInsp = insp.rows[0];
    console.log("--- LATEST INSPECCION ---");
