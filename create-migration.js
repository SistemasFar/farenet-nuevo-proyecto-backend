...
fs.writeFileSync('modules/faregas/database/migrations/20260914_faregas_ancho_correlativo.sql', migrationSql);

const runMigration = `const db = require('./config/database');
async function run() {
    try {
...
fs.writeFileSync('run-migration.js', runMigration);
