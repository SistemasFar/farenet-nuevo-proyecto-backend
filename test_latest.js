...
async function testLatest() {
  try {
    const insp = await db.query("SELECT nrodocumentoinspeccion FROM inspeccion ORDER BY fechcreacion DESC LIMIT 1");
    if (!insp.rows[0]) {
      console.log("NO INSPECTIONS FOUND");
      process.exit(0);
    }
    const nro = insp.rows[0].nrodocumentoinspeccion;
    console.log("LATEST INSPECTION:", nro);
    ...
    `;
    const res = await db.query(query, [nro]);
