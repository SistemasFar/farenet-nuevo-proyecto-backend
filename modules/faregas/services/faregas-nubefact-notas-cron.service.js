...
const reconciliarNotasPendientesSunat = async (opciones = {}) => {
    ...
    try {
        ...
        const query = `
            SELECT f.id as documento_id, f.certificado_id, f.estado, f.serie, f.numero, f.tipo_comprobante, f.operacion_id,
                   c.planta_key
            FROM fg_credito f
        ...
        if (pendientesRows.length > 0) {
            const ids = pendientesRows.map(r => r.documento_id);
            await client.query(`
                UPDATE fg_credito 
    ...
    for (const fila of pendientesRows) {
        ...
        try {
            ...
            console.error(`[CRON NUBEFACT] Error reconciliando facturacion ${fila.documento_id}:`, err.message);
        }
    }
...
module.exports = { reconciliarNotasPendientesSunat };
