...
const isTestEnv = process.env.FAREGAS_TEST_DB_ENABLED === 'true' &&
    (process.env.APP_ENV_FILE === '.env.test' || (process.env.APP_ENV_FILE || '').toUpperCase().includes('TEST')) &&
    process.env.APP_DATABASE_ENVIRONMENT === 'TEST';

if (!isTestEnv) {
    test('Configuración modifica inmediatamente el catálogo y rollback restaura los datos', { skip: 'Faltan restricciones TEST' }, () => {});
    test('TEST_COMPLEMENTARIO existe en Configuración pero queda fuera de Nuevo Certificado y del backend', { skip: 'Faltan restricciones TEST' }, () => {});
} else {
