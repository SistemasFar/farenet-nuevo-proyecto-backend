...
const serviceCode = \const db = require('../../../config/database');
...
const faregasFormatosService = {
  listarFormatos: async () => {
    const query = "SELECT f.id, f.codigo, f.nombre, f.motor, f.es_protegido, f.activo, (SELECT COUNT(*) FROM fg_certificado_formato_version v WHERE v.formato_id = f.id AND v.estado = 'VIGENTE') > 0 as tiene_version_vigente FROM fg_certificado_formato f ORDER BY f.id ASC";
    const res = await db.query(query);
    return res.rows;
  },

  obtenerVersionesFormato: async (formatoId) => {
    const query = "SELECT id, version, configuracion, estado, vigente_desde, creado_en FROM fg_certificado_formato_version WHERE formato_id =  ORDER BY version DESC";
    const res = await db.query(query, [formatoId]);
    return res.rows;
  },

  crearFormato: async (nombre, codigo, motor) => {
    const query = "INSERT INTO fg_certificado_formato (nombre, codigo, motor, es_protegido, activo) VALUES (, , , false, true) RETURNING *";
  ...
  guardarBorradorVersion: async (formatoId, fileData, originalName) => {
    ...
    const formatoRes = await db.query('SELECT * FROM fg_certificado_formato WHERE id = ', [formatoId]);
    ...
    try {
        ...
        const verRes = await client.query('SELECT COALESCE(MAX(version), 0) + 1 as next_v FROM fg_certificado_formato_version WHERE formato_id = ', [formatoId]);
        ...
        fs.writeFileSync(templatePath, fileData);

        const storageKey = path.join(formato.codigo, 'v' + nextVersion).replace(/\\\\/g, '/');
        ...
        const insertQuery = "INSERT INTO fg_certificado_formato_version (formato_id, version, archivo_ruta, configuracion, estado) VALUES (, , , , 'BORRADOR') RETURNING id, version, estado";
  ...
  obtenerEstructura: async (formatoId, versionId) => {
    const verRes = await db.query('SELECT archivo_ruta FROM fg_certificado_formato_version WHERE id =  AND formato_id = ', [versionId, formatoId]);
  ...
  guardarMappings: async (formatoId, versionId, mappings) => {
    const verRes = await db.query('SELECT archivo_ruta, estado FROM fg_certificado_formato_version WHERE id =  AND formato_id = ', [versionId, formatoId]);
    if (verRes.rowCount === 0) throw new Error('Versión no encontrada');
    ...
    await db.query('UPDATE fg_certificado_formato_version SET configuracion =  WHERE id = ', [
  ...
  activarVersion: async (formatoId, versionId) => {
    ...
    try {
        ...
        const verRes = await client.query('SELECT estado FROM fg_certificado_formato_version WHERE id =  AND formato_id =  FOR UPDATE', [versionId, formatoId]);
        if (verRes.rowCount === 0) throw new Error('Versión no encontrada');
        if (verRes.rows[0].estado !== 'BORRADOR') throw new Error('La versión no está en estado BORRADOR');
        
        await client.query("UPDATE fg_certificado_formato_version SET estado = 'RETIRADA' WHERE formato_id =  AND estado = 'VIGENTE'", [formatoId]);
        
        await client.query("UPDATE fg_certificado_formato_version SET estado = 'VIGENTE', vigente_desde = NOW() WHERE id = ", [versionId]);
  ...
  renderVersion: async (formatoVersionId, data) => {
    const verRes = await db.query('SELECT archivo_ruta, estado FROM fg_certificado_formato_version WHERE id = ', [formatoVersionId]);
    if (verRes.rowCount === 0) throw new Error('Versión no encontrada');
...
module.exports = { faregasFormatosService };
\;

fs.writeFileSync('c:/Users/Sistemas2/Desktop/farenet nuevo proyecto/farenetBackend/modules/faregas/services/faregas-formatos.service.js', serviceCode, 'utf8');
