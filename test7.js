...
const test = async () => {
  try {
    const query = `
      SELECT cam.*, vd.id AS verificaciondescuento_id, vd.codigo AS verificaciondescuento_codigo, cd.valordescuento AS monto 
      FROM campania cam
      ...
      WHERE pl.key = '201' AND ci.key = '34' AND cam.estado = true 
